...
namespace Toon
{
	...
	****************************************************************************/
	...
	{
	public:
		static unsigned long hashRuntime(char const*, unsigned long long);
	private:
		static inline constexpr unsigned long UPDC32(char octet, unsigned long crc)
		{
			return (crc32Table[((crc) ^ ((unsigned char)octet)) & 0xff] ^ ((crc) >> 8));
		}
		
		/* 
			below members and methods are for compile-time hash function.
			but, compile-time hash function is very heavy for long string.
			it is only useful for short string.
		*/
		enum { CRC32TABLE_SIZE = 256 };
		/* Copyright (C) 1986 Gary S. Brown.  You may use this program, or
		code or tables extracted from it, as desired without restriction.*/
		static constexpr std::array<unsigned long, CRC32TABLE_SIZE> crc32Table { /* CRC polynomial 0xedb88320 */
	...
	public:
		...
		static constexpr unsigned long hashConstexpr(char const*);
	};
	...
	template <>
	constexpr unsigned long CRC32Hasher::hashConstexpr<0U>(char const*)
	{
		return 0xFFFFFFFF;
	}
};
